...
input_gt_dir = 'test_gt_1080p_v1011_sess_comb'
...
for txt in gt_list:
    ...
    gt_labels_qtr = [i for i in gt_labels if i not in qtr_list]
    pred_labels_qtr = [i for i in pred_labels if i not in qtr_list]

    gt_labels_comb = [rm_1st_digit(i) for i in gt_labels if (contain_eng(i) == 1 and (i not in qtr_list)) or (':' in i)]
    pred_labels_comb = [rm_1st_digit(i) for i in pred_labels if (contain_eng(i) == 1 and (i not in qtr_list)) or (':' in i)]
    ...
    if inter_num(gt_labels_comb,pred_labels_comb) >= len(gt_labels_comb):
        i_c = i_c+1
        print('  ',gt_labels_comb)
        print('->',pred_labels_comb)



#print('比分和时间完全相同（小框*4）：',i_4/s)
#print('队名/比分和时间完全相同（小框*6）：',i_6/s)
print('队名_比分和时间（小框*5）：',i_c/s)
